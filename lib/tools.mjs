...
const BUILTIN_TOOLS = [
  ...
  { id: 'cursor', name: 'Cursor', command: 'cursor-agent', runtimeFamily: 'cursor-stream-json' },
...
const SIMPLE_RUNTIME_FAMILIES = new Set(['claude-stream-json', 'codex-json', 'cursor-stream-json']);
...
export async function saveSimpleToolAsync({ name, command, runtimeFamily, models, reasoning, visibility }) {
  ...
  if (!normalizedFamily) {
    throw new Error('runtimeFamily must be one of: claude-stream-json, codex-json, cursor-stream-json');
  }
